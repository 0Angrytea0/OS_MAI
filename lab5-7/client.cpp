...
int main(int argc, char* argv[]){
    ...
    rc = zmq_setsockopt(subscriber, ZMQ_SUBSCRIBE,"",0); //установка подписи, нужно, чтобы получать сообщения
    assert(rc == 0);
    ...
    for(;;){
        ...
        if(!(zmq_msg_recv (&reply, subscriber, 0))){
            std::cout << "Error: can't get msg" << std::endl;
        }
        ...
        if (data->command == 1){
            ...
            if(data->node != CURRENT_NODE){
                if(!(zmq_msg_send(&reply, publisher, 0))){
            ...
            if(data->node == CURRENT_NODE || data->node == -1){
                ...
                if(!(zmq_msg_send(&message, publisher, 0))){
                    std::cout << "Error: can't send" << std::endl;
                }
