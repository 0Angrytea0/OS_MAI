...
int create_node(int id, int parent)
{
    ...
    if(execl("./client", "./client", arg1, arg2, NULL) == -1){
        return -1;
    }
    return 1;
}
...
int main(){
    ...
    std::vector<int> buzy_node; //кол-во занятых делом  
    
    void* context = zmq_ctx_new();
    void* publiser = zmq_socket(context, ZMQ_PUB);
    int r = zmq_bind(publiser, "tcp://*:5000");
    if(r == -1){
    ...
    while(true){
        ...
        if(command == "create"){
            ...
            if(!(zmq_msg_send(&message, publiser, 0))){
